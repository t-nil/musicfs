...
/// Find music files in `root` and create internal structures.
/// Call inside init.
...
impl MusicFS {
    pub fn from_dir(root: impl AsRef<Path>) -> (Self, Vec<Error>) {
        let files_matching_ext = WalkDir::new(&root)
            ...
            .into_iter()
            .filter_ok(is_media_file_by_name)
            .collect_vec();

        let probed_files = files_matching_ext
            .iter()
            .map(Result::as_ref)
            .map_ok(DirEntry::path)
            .map_ok(types::File::new)
            .flatten()
            .map_ok(MusicFile::parse_with_symphonia)
            .flatten();

        let (source_files, errors): (Vec<MusicFile>, Vec<Error>) = probed_files.partition_result();
